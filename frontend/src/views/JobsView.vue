  ...
  <div class="jobs-view">
    <div class="page-heading jobs-heading">
      <div>
        ...
        <h1>Joblarım</h1>
        <span
          >Uzun süren işlemler arka planda çalışır. Burada yalnızca size ait joblar
          gösterilir.</span
        >
      </div>
    ...
    <n-grid :cols="3" :x-gap="16" responsive="screen" item-responsive>
      <n-grid-item span="3 m:1"
        ><n-card size="small"
          ><n-statistic label="Sırada / çalışıyor" :value="counts.active" /></n-card
      ></n-grid-item>
      <n-grid-item span="3 m:1"
        ><n-card size="small"><n-statistic label="Tamamlanan" :value="counts.completed" /></n-card
      ></n-grid-item>
      <n-grid-item span="3 m:1"
        ><n-card size="small"><n-statistic label="Başarısız" :value="counts.failed" /></n-card
      ></n-grid-item>
    </n-grid>
            ...
            <div class="job-row">
              <div class="job-main">
                <n-space align="center">
                  ...
                  <n-tag size="small" :type="statusType(job.status)">{{
                    job.status_display
                  }}</n-tag>
                </n-space>
                <span>{{ job.document_name || `Job ${job.id}` }}</span>
                <small
                  >{{ formatDate(job.created_at) }} · Deneme {{ job.attempts }}/{{
                    job.max_attempts
                  }}</small
                >
                <n-progress
              ...
              <n-space class="job-actions">
                <n-button
                  ...
                  size="small"
                  secondary
                  @click="emit('open-document', job.document)"
                  >Belgeyi aç</n-button
                >
                <n-button
