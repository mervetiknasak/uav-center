    ...
    <section class="upload-panel">
      <n-card title="Belge Yükle" size="small">
        <n-space vertical :size="16">
          ...
          <n-alert v-if="uploadNotice" type="info" title="İşlem sıraya alındı">{{
            uploadNotice
          }}</n-alert>
        </n-space>
      </n-card>
      ...
      <n-card title="Doküman Kontrolleri" size="small">
        <n-spin :show="controlsLoading">
          <n-space vertical :size="12">
              ...
              <n-space vertical>
                <div v-for="control in controls" :key="control.id" class="control-row">
                  ...
                  <n-space v-if="control.kind === 'custom'" :size="4">
                    <n-button size="tiny" quaternary @click="editControl(control)"
                      >Düzenle</n-button
                    >
                    <n-button
                      size="tiny"
                      quaternary
                      type="error"
                      @click="emit('delete-control', control)"
                      >Sil</n-button
                    >
                  </n-space>
            ...
            <n-input
              v-model:value="controlForm.description"
            ...
            <n-space justify="end">
              ...
              <n-button
                type="primary"
                :disabled="!controlForm.name.trim() || controlForm.instructions.trim().length < 10"
                @click="submitControl"
              >
      ...
      <n-card title="AI İşleme Sonucu" size="small">
        ...
        <n-space v-else vertical :size="16">
          ...
          >
            ...
            <span v-else-if="activeDocument.status === 'pending'"
              >{{ activeDocument.original_name }} arka planda işlenmek üzere sıraya alındı.</span
            >
            <span v-else
              >{{ activeDocument.original_name }} içeriği yerelde çıkarıldı{{
                activeDocument.ai_result?.ai_enabled ? " ve AI ile işlendi" : ""
              }}.</span
            >
          </n-alert>
          <n-descriptions :column="2" bordered size="small">
            ...
            <n-descriptions-item label="Sağlayıcı">{{
              activeDocument.ai_result?.ai_enabled === false
                ? "Devre dışı"
          ...
          <template v-if="activeDocument.ai_result?.ai_enabled !== false">
            <div>
              ...
              <p class="summary-text">
                {{ activeDocument.prompt || activeDocument.ai_result?.prompt || "-" }}
              </p>
            </div>
            <div>
              <h2>Model Yanıtı</h2>
              <p class="summary-text">
                {{ activeDocument.ai_result?.response || activeDocument.ai_result?.summary || "-" }}
              </p>
            </div>
          </template>
          ...
          <n-card
            v-if="activeDocument.ai_result?.ocr?.enabled"
            title="OCR Sonucu"
            size="small"
            embedded
          >
            <n-space vertical :size="12">
              <n-descriptions :column="2" bordered size="small">
                ...
                <n-descriptions-item label="Diller">{{
                  activeDocument.ai_result.ocr.languages.join(", ").toUpperCase()
                }}</n-descriptions-item>
                <n-descriptions-item label="Görsel">{{
                  activeDocument.ai_result.ocr.processed_images
                }}</n-descriptions-item>
                <n-descriptions-item label="PDF sayfası">{{
                  activeDocument.ai_result.ocr.processed_pages
                }}</n-descriptions-item>
              </n-descriptions>
              ...
              />
              <n-space v-else>
                <n-tag v-for="address in emailAddresses" :key="address" type="success">{{
                  address
                }}</n-tag>
              </n-space>
              <n-alert
                v-if="activeDocument.ai_result.ocr.warnings.length"
                type="warning"
                title="Bazı içerikler OCR ile okunamadı"
              >
                <ul class="ocr-warning-list">
                  <li v-for="warning in activeDocument.ai_result.ocr.warnings" :key="warning">
                    {{ warning }}
                  </li>
                </ul>
              </n-alert>
          ...
          <n-space justify="end">
            <n-button
              ...
              :disabled="!documentReady || !ragQuery.trim()"
              @click="emit('ask-document')"
            >
              Kaynaklarla yanıtla
            </n-button>
            <n-button
              ...
              @click="emit('run-controls')"
              >Seçili kontrolleri çalıştır</n-button
            >
          </n-space>
          <n-alert v-if="analysisError" type="error" title="Analiz hatası">{{
            analysisError
          }}</n-alert>
          <n-card v-if="ragResult" title="RAG yanıtı" embedded size="small">
            ...
            <n-collapse v-if="ragResult.sources?.length">
              <n-collapse-item
                :title="`Kaynaklar (${ragResult.sources.length})`"
                name="rag-sources"
              >
                <n-list>
                  <n-list-item v-for="source in ragResult.sources" :key="source.id">
                    <n-thing
                      :title="`${source.id} · ${source.document_name}`"
              ...
              <n-list-item v-for="result in controlResult.controls" :key="result.id">
                <n-thing :title="result.name" :description="result.summary">
                  <template #header-extra
                    ><n-tag :type="outcomeType(result.outcome)">{{
                      result.outcome
                    }}</n-tag></template
                  >
                </n-thing>
